...
## ENGLISH 
colnames(symptoms) <- c("Headache", "Pressure in head", "Neck pain", "Nausea or vomiting", "Dizziness", "Blurred vision", "Balance problems", "Sensitivity to light", "Sensitivity to noise", "Feeling slowed down", "Feeling like in a fog", "Dont feel right", "Difficulty concentrating", "Difficulty remembering", "Fatigue or low energy", "Confusion", "Drowsiness", "More emotional", "Irritability", "Sadness", "Nervous or anxious", "Trouble falling asleep")

  symptoms <- symptoms[,order(symptoms)]
  par(mar=c(4,7,4,4))
  barplot(symptoms, horiz=T, las=1, xlab="Symptom Score (0 = none, 1 & 2 = mild, 3 & 4 = moderate, 6 = severe)", xlim = c(0,6), cex.names=0.50, space=c(1))
  title(main = "Symptom Evaluation at Baseline", cex.main = 0.9)
  print(paste ("Total Symptom Severity Score: ", symptom_score, sep=""))
...
```{r domains_, echo=FALSE, warning = FALSE, results='asis'}
...
test <- et_select_imp[which(et_select_imp$group==1),]$test
...
ggplot(data = et_domains_all_long, aes(x=Score, y=Domains, group = Date, colour=Date)) + geom_rect(aes(xmin = -1, xmax = 1), ymin = ystart, ymax = yend, alpha = 0.05, fill="green", colour=NA) + geom_rect(aes(xmin = -2, xmax = -1), ymin = ystart, ymax = yend, alpha = 0.05, fill="yellow", colour=NA) + geom_rect(aes(xmin = -Inf, xmax = -2), ymin = ystart, ymax = yend, alpha = 0.05, fill="red", colour=NA) + geom_rect(aes(xmin = 1, xmax = 2), ymin = ystart, ymax = yend, alpha = 0.10, fill="green", colour=NA) + geom_rect(aes(xmin = 2, xmax = Inf), ymin = ystart, ymax = yend, alpha = 0.15, fill="green", colour=NA) + geom_errorbarh(aes(x = Score, xmin=Score-0.75, xmax=Score+0.75), width=.2, size =.3, position = position_dodgev(height = .5)) + geom_point(size=3, position = position_dodgev(height = .5)) + ggtitle("Domain Exploration - Information Processing") + theme(plot.title = element_text(hjust = 0.5)) + geom_vline(xintercept = c(0), color = "grey", size=0.5) + xlab ("Performance (z-score ± .75 SD)") + ylab("Neurocognitive Function")


# define variables
vars <- c("mrt_neutral", "sd_neutral", "tau")
dirs <- c("pos", "pos", "pos")
des <- c("Processing speed", "Processing Stability", "Motor Response Inhibition")
trrs <- c(NA, NA, NA)
...
ggplot(data = et_domains_all_long, aes(x=Score, y=Domains, group = Date, colour=Date)) + geom_rect(aes(xmin = -1, xmax = 1), ymin = ystart, ymax = yend, alpha = 0.05, fill="green", colour=NA) + geom_rect(aes(xmin = -2, xmax = -1), ymin = ystart, ymax = yend, alpha = 0.05, fill="yellow", colour=NA) + geom_rect(aes(xmin = -Inf, xmax = -2), ymin = ystart, ymax = yend, alpha = 0.05, fill="red", colour=NA) + geom_rect(aes(xmin = 1, xmax = 2), ymin = ystart, ymax = yend, alpha = 0.10, fill="green", colour=NA) + geom_rect(aes(xmin = 2, xmax = Inf), ymin = ystart, ymax = yend, alpha = 0.15, fill="green", colour=NA) + geom_errorbarh(aes(x = Score, xmin=Score-0.75, xmax=Score+0.75), width=.2, size =.3, position = position_dodgev(height = .5)) + geom_point(size=3, position = position_dodgev(height = .5)) + ggtitle("Domain Exploration - Attention Control") + theme(plot.title = element_text(hjust = 0.5)) + geom_vline(xintercept = c(0), color = "grey", size=0.5) + xlab ("Performance (z-score ± .75 SD)") + ylab("Neurocognitive Function")


#### RCI ####

# define variables
vars <- c("alerting_mrt", "orienting_mrt", "exec_mrt", "exec_acc")
...
if(center!="Ajax"){
    ...
    et_domains_all_sel <- select(et_domains_all, c(test, vis_mem_enc))
    colnames(et_domains_all_sel) <- c("Date", "Visual Encoding")
    }
