...
# get patient data
#et_exp <- read.table("../emma_toolbox_data/Databases/ET_DATABASE_CM.txt", header = TRUE, sep=" ")
et_exp <- read.table("../data/ET_DATABASE_CM.txt", header = TRUE, sep=" ")
...
if (exists("bu")){
  et_exp$subj <- as.numeric(substr(et_exp$subj, 1, 4))
  subj_base <- as.numeric(substr(subj, 1, 4))
} else{
  subj_base <- subj
}
...
# PREPROCESS
rmarkdown::render("Scripting/subscripts/subscripts/preprocess_elite.Rmd")


et_domains <- et_select_comp
et_domains <- et_domains %>% separate(subj, c("subj", "test"), sep="_")


# APPLY NORM REGRESSION
#ctrl_demo <- read.table(paste0("../emma_toolbox_data/Norms/", pop, "_age.txt"), header = TRUE)
ctrl_demo <- read.table(paste0("../norm_data/KNVB_elite_age.txt"), header = TRUE)
...
rmarkdown::render("Scripting/subscripts/subscripts/report_generator_elite.Rmd", output_file = paste0("../../../../emma_toolbox_data/Files/", subj, "/", emmadate, "/Reports/Emma Toolbox - Report.html", sep=""))}
