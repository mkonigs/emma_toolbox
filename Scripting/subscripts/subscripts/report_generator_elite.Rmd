...
std <- sd(eval(parse(text = paste0("ctrl_normed$", var))), na.rm=TRUE)  
...
rci_results <- list("sd_change" = sd_change)
return(rci_results)
}
...
ass <- select(et_exp_, c("test", "meting"))
ages <- merge(ages, ass, by="test")
...
if(center!="Ajax"){
  et_domains_all_sel <- select(et_domains_all, c(test, exec_acc, exec_mrt, orienting_mrt, alerting_mrt, tau, sd_neutral, mrt_neutral))
  colnames(et_domains_all_sel) <- c("Date", "Interference Control (accuracy)", "Interference Control (mrt)", "Orienting Attention", "Alerting Attention" ,"Processing Consistency", "Processing Stability", "Processing Speed")}else{
...
# define variables
if(center!="Ajax"){
  vars <- c("mrt_neutral", "sd_neutral", "tau", "alerting_mrt", "orienting_mrt", "exec_mrt", "exec_acc")
  dirs <- c("pos", "pos", "pos", "pos", "pos", "pos", "pos")
  des <- c("Processing Speed", "Processing Stability", "Processing Consistency", "Alerting Attention", "Orienting Attention", "Interference Control (accuracy)", "Interference Control (mrt)")
  trrs <- c(NA, NA, NA, NA, NA, NA, NA)
  scales <-c("z-score", "z-score", "z-score", "z-score", "z-score", "z-score", "z-score")
...
rci_table <- data.frame(matrix(NA, (nrow=nrow(ET_subj)*(nrow(ET_subj)-1)/2), ncol=2))
colnames(rci_table) <- c("Timepoint", "SD change")
...
print(kable(rci_table, format="markdown"))
