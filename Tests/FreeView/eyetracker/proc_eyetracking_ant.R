...
calculateAll <- function(G.df, subject.id) {
  ...
  nr_trials_condition <- G.df %>% 
    ...
    ungroup() %>% 
    summarise(congruent.trials.n = sum(con.trials), incongruent.trials.n = sum(incon.trials),
  ...
  meanSacVel <- G.df %>%
    ...
    ungroup(G.df) %>% 
    summarise(n_trials.3 = length(unique(trial.number)), n.saccades.3 = length(trial.number),
  ...
  meanPeakSacVel <- G.df %>% 
    ...
    ungroup(G.df) %>% 
    summarise(n_trials.4 = length(unique(trial.number)), avgPeakVelSac.4 = mean(max_v))
  ...
  meanPeakAcc <- G.df %>% 
    ...
    ungroup(G.df) %>% 
    summarise(n_trials.5 = length(unique(trial.number)), mean_peak_acc.5 = mean(max_a))
